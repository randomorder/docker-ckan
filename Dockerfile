...
# Configure CKAN
RUN ln -s "${CKAN_HOME}/ckan/ckan/config/who.ini" "${CKAN_CONFIG}/who.ini"


ADD ckan-entrypoint.sh /ckan-entrypoint.sh
RUN chmod +x /ckan-entrypoint.sh
RUN chown ckan:ckan "${CKAN_HOME}" -R && chown ckan:ckan "${CKAN_CONFIG}" -R && chown ckan:ckan "${CKAN_STORAGE_PATH}" -R
